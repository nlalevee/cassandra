...
public class CassandraManager
{
    ...
    public KsDef createKeyspaceWithColumns() throws MetaException
    {
        ...
        {
            throw new MetaException("Unable to create key space '" + keyspace + "'. Error:" + e.getMessage());
        } catch (InvalidRequestException e)
        {
            throw new MetaException("Unable to create key space '" + keyspace + "'. Error:" + e.getMessage());
    ...
    /**
     * Get the value for a given name from the table. It first checks the table property. If it is not there, it checks
    ...
    public void dropTable() throws MetaException
    {
        ...
        {
            throw new MetaException("Unable to drop column family '" + columnFamilyName + "'. Error:" + e.getMessage());
        } catch (InvalidRequestException e)
        {
            throw new MetaException("Unable to drop column family '" + columnFamilyName + "'. Error:" + e.getMessage());
        } catch (SchemaDisagreementException e)
