 ...
 */
package org.apache.cassandra.hadoop.hive;
...
public class CassandraProxyClient implements java.lang.reflect.InvocationHandler
{
    ...
    /**
     * Option to choose the next server from the ring. Default is RoundRobin unless specified by the client to choose
     * randomizer.
     */
    ...
    /**
     * Initialize the cassandra connection with the initial given cassandra host. Create a temporary keyspace if no one
     * exists. Otherwise, choose the first non-system keyspace to describe the ring. Initialize the ring.
     * 
     * @throws IOException
     */
    ...
    /**
     * Create a temporary keyspace. This will only be called when there is no keyspace except system defined on (new
     * cluster). However we need a keyspace to call describe_ring to get all servers from the ring.
     * 
    ...
    /**
     * Attempt to connect to the next available server. If there is no server in the ring, an exception will be thrown
     * out. If there is no server in the ring that is different from the server used last time, we should try the same
     * server again in case that it recovers. Otherwise, try to connect to a different server.
     * 
     * @throws error
    ...
    public abstract class RingConnOption
    {
        ...
        /**
         * Return the next server from the ring. If there is no server in the ring, throw an exception. If there is only
         * one server in the ring, return the server if it is different from the server tried last time. If there are
         * more than two servers in the ring, return the server that is different from the server tried last time; if
         * there is no server that is different from the server tried last time, return null;
         * 
         * @param the
         ...
         * 
         * In the constructor, all servers from the ring are hashed and mapped. Theoretically there should be no
         * duplicated server in the ring.
         * 
         * @param host
