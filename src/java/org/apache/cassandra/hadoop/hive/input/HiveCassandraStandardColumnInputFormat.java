...
public class HiveCassandraStandardColumnInputFormat extends ColumnFamilyInputFormat implements
        InputFormat<Text, HiveCassandraStandardRowResult>
{
    ...
    @Override
    public RecordReader<Text, HiveCassandraStandardRowResult> getRecordReader(InputSplit split, JobConf jobConf,
            final Reporter reporter) throws IOException
    {
        HiveCassandraStandardSplit cassandraSplit = (HiveCassandraStandardSplit) split;

        List<String> columns = StandardColumnSerDe.parseColumnMapping(cassandraSplit.getColumnMapping());
        ...
        try
        {
            ConfigHelper.setInputColumnFamily(tac.getConfiguration(), cassandraSplit.getKeyspace(),
                    cassandraSplit.getColumnFamily());
                ...
                try
                {
                    ...
                    if (isTransposed)
                    {
                        ...
                        if (next)
                        {
                            ...
                            theMap.put(new BytesWritable(StandardColumnSerDe.CASSANDRA_COLUMN_COLUMN.getBytes()), hic);
                            ...
                            if (superColumn)
                            {
                                ...
                                theMap.put(
                                        new BytesWritable(StandardColumnSerDe.CASSANDRA_SUBCOLUMN_COLUMN.getBytes()),
                                        hic);
                                ...
                                theMap.put(new BytesWritable(StandardColumnSerDe.CASSANDRA_VALUE_COLUMN.getBytes()),
                                        hic);
    ...
    /**
     * The Cassandra record Reader throws InteruptedException, we overlay here to throw IOException instead.
     */
    @Override
    ...
    public InputSplit[] getSplits(JobConf jobConf, int numSplits) throws IOException
    {
        ...
        int sliceRangeSize = jobConf.getInt(StandardColumnSerDe.CASSANDRA_RANGE_BATCH_SIZE,
                StandardColumnSerDe.DEFAULT_RANGE_BATCH_SIZE);
        int splitSize = jobConf
                .getInt(StandardColumnSerDe.CASSANDRA_SPLIT_SIZE, StandardColumnSerDe.DEFAULT_SPLIT_SIZE);
    ...
    /**
     * Return a list of columns names to read from cassandra. The column defined as the key in the column mapping should
     * be skipped.
     * 
     * @param iKey
