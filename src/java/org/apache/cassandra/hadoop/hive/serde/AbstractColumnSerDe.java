...
import org.apache.cassandra.hadoop.hive.input.HiveCassandraStandardRowResult;
import org.apache.cassandra.hadoop.hive.input.LazyCassandraRow;
...
public abstract class AbstractColumnSerDe implements SerDe
{
    ...
    public static final String CASSANDRA_KEYSPACE_STRATEGY = "cassandra.ks.strategy"; // keyspace replica placement
                                                                                      // strategy
    ...
    public static final String CASSANDRA_BATCH_MUTATION_PAUSE = "cassandra.batchmutate.pause";
    ...
    public static final int DEFAULT_BATCH_MUTATION_PAUSE = 100;
    ...
    public void initialize(Configuration conf, Properties tbl) throws SerDeException
    {
        ...
        cachedCassandraRow = new LazyCassandraRow((LazySimpleStructObjectInspector) cachedObjectInspector);

        if (LOG.isDebugEnabled())
        {
            LOG.debug("CassandraSerDe initialized with : columnNames = "
                    + StringUtils.join(serdeParams.getColumnNames(), ",") + " columnTypes = "
                    + StringUtils.join(serdeParams.getColumnTypes(), ",") + " cassandraColumnMapping = "
    ...
    /*
     * 
     * @see org.apache.hadoop.hive.serde2.Deserializer#deserialize(org.apache.hadoop.io.Writable) Turns a Cassandra row
     * into a Hive row.
     */
    @Override
    ...
    /**
     * Parses the cassandra columns mapping to identify the column name. One of the Hive table columns maps to the
     * cassandra row key, by default the first column.
     * 
     * @param columnMapping
     *            - the column mapping specification to be parsed
    ...
    /*
     * Creates the cassandra column mappings from the hive column names. This would be triggered when no
     * cassandra.columns.mapping has been defined in the user query.
     * 
     * row_key is a special column name, it maps to the key of a row in cassandra; column_name maps to the name of a
     * column/supercolumn; value maps to the value of a column; sub_column_name maps to the name of a column (This can
     * only be used for a super column family.)
     * 
    ...
    protected String getCassandraColumnFamily(Properties tbl) throws SerDeException
    {
        ...
        if (result == null)
        {
            ...
            result = tbl.getProperty(org.apache.hadoop.hive.metastore.api.Constants.META_TABLE_NAME);
    ...
    /**
     * Parse the column mappping from table properties. If cassandra.columns.mapping is defined in the property, use it
     * to create the mapping. Otherwise, create the mapping from table columns using the default mapping.
     * 
     * @param tbl
     *            table properties
    ...
    /**
     * Return if a table is a transposed. A table is transposed when the column mapping is like (:key, :column, :value)
     * or (:key, :column, :subcolumn, :value).
     * 
     * @param column
     *            mapping
